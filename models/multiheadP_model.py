...
class SimpleModel(BaseModel):
    ...
    def forward(self):
        self.est_mask, self.est_contour, self.real_contour = self.net.forward(self.input_im, self.real_mask)

    def calc_loss(self):
        self.loss = self.criterion_parts(self.est_mask, self.real_mask) + \
                    self.criterion_contour(self.est_contour, torch.argmax(self.real_contour, dim=1)) + self.criterion_corr(self.est_contour,
                                                                                                      self.est_mask)
